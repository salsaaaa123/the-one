	...
	private class TupleComparator implements Comparator<Tuple<Message, Connection>> {
		...
		public int compare(Tuple<Message, Connection> tuple1,
			...
			if (p2 - p1 == 0) {
				...
				/* equal probabilities -> break tie using message receive time (FIFO-like) */
				// double time1 = tuple1.getKey().getReceiveTime(); // Gunakan waktu terima
				// double time2 = tuple2.getKey().getReceiveTime(); // Gunakan waktu terima

				// if (time1 < time2)
				// return -1; // Pesan yang diterima lebih dulu (<) didahulukan (-1)
				// if (time1 > time2)
				// return 1;
				// return 0; // Waktu terima sama
			} else if (p2 - p1 < 0) {
				return -1;
			} else {
				return 1;
