...
public class VRRouter extends ActiveRouter {
	...
	public static final String MESSAGE_CHECKIN_SEQNUM_P = "VRRouter.checkInID";
	public static final String MESSAGE_FOCUS_P = "VRRouter.focus";
	public static final String MESSAGE_IS_CHECKIN_P = "VRRouter.isCheckIn";
	public static final String MESSAGE_HOME_LOC_P = "VRRouter.homeLocation";
	public static final String MESSAGE_LOOK_P = "VRRouter.look";
	 ...
	 * 
	 * Ellipse2D.Double repArea = new
	 * Ellipse2D.Double(PlayFieldGraphic.scale(home.getX()-rad),
	 * PlayFieldGraphic.scale(home.getY()-rad), PlayFieldGraphic.scale(2*rad),
	 * PlayFieldGraphic.scale(2*rad));
	 * g2.setColor(Color.blue);
	 * g2.draw3DRect(PlayFieldGraphic.scale(home.getX()-1),
	 * PlayFieldGraphic.scale(home.getY()-1),
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		// System.out.println(vrm.toString() + " transfered to host: " +
		// getHost().getAddress());
		 ...
		 */
		((VRRouter) from.getRouter()).ackMessage(vrm, getHost());
	 ...
	 * as an acknowledgment for the message. The peer passes in a reference to
	 * itself
	...
	public void update() {
		...
		java.util.List<Tuple<Message, Connection>> messages = new LinkedList<Tuple<Message, Connection>>();
		...
		for (Message m : msgCollection) {
			...
			Coord msgDest;
			boolean isDestOutsideNeighborhood = !neighborhood.containsKey(to) || isCheckInMessage(m);
			...
			// for(Connection c : getConnections())
			{
				...
				if (distToMsgDest > peerDistToMsgDest) {
					 ...
					 * " is not");
					 * if(peerRelMovingTowards || peerDistToMsgDest < distToMsgDest &&
					 * !myMovingTowards)
	...
	private void updateNeighborhood(double time) {
		...
		for (Iterator<Map.Entry<DTNHost, NeighborEntry>> i = neighborhood.entrySet().iterator(); i.hasNext();) {
