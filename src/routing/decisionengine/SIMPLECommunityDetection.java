 ...
 *                         Title = {A socio-aware overlay for publish/subscribe
 *                         communication in
 *                         delay tolerant networks},
 *                         Address = {New York, NY, USA},
 *                         Author = {Yoneki, Eiko and Hui, Pan and Chan, ShuYan
 *                         and Crowcroft, Jon},
 *                         Booktitle = {MSWiM '07: Proceedings of the 10th ACM
...
public class SIMPLECommunityDetection implements RoutingDecisionEngine, CommunityDetectionEngine {
	...
	public void doExchangeForNewConnection(Connection con, DTNHost peer) {
		...
		if (!this.localCommunity.contains(peer)) {
			...
			for (DTNHost h : de.familiarSet)
				if (this.localCommunity.contains(h))
					count++;
			// if(count > 0)
			// System.out.println(myHost.toString() + " count: " + count + " peerSize: " +
			// peerFsize);
			if (addPeerToMyLocal = ((double) count) / peerFsize > this.lambda) {
				// System.out.println(myHost.toString() + " adding " + peer + " to Local
				// Community");
				this.localCommunity.add(peer);
			}
		}
		...
		if (!de.localCommunity.contains(myHost)) {
			...
			if (addMeToPeerLocal = ((double) count) / myFsize > de.lambda) {
				// System.out.println(peer.toString() + " adding " + myHost + " to Local
				// Community");
	...
	public Set<DTNHost> getFamiliars() {
